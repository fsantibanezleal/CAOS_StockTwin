import { Callout, Cite, InlineMath, Refs, useShellLang } from '@fasl-work/caos-app-shell';
...
export default function Introduction() {
  ...
  return (
    <div className="page-body prose">
      ...
      <section>
        <h2>{es ? '1. El objeto, dicho con precisión' : '1. The object, said precisely'}</h2>
        <p>
          {es
      ...
      <section>
        <h2>{es ? '2. Como se construye un banco' : '2. How a bench gets built'}</h2>
        <p>
          {es
      ...
      <section>
        <h2>{es ? '3. Cuatro formas medidas, no una supuesta' : '3. Four measured shapes, not one assumed'}</h2>
        <p>
          {es
      ...
      <section>
        <h2>{es ? '4. Por que vale la pena rastrear cada carga' : '4. Why tracking every load is worth anything'}</h2>
        <p>
          {es
      ...
      <section>
        <h2>{es ? '5. Qué NO es' : '5. What it is not'}</h2>
        <ul>
          <li>
            {es
