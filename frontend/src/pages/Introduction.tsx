  ...
  return (
    <div className="page-body prose">
      ...
      <section>
        ...
        <p>
          {es
            ? 'Una losa de mineral de mina (ROM), una pila de mineral grueso despues del chancado primario, o una cama de mezcla longitudinal en un patio de carbon o de cemento: los tres son el mismo objeto fisico visto con distintos nombres. Un apilador deposita material a lo largo de una trayectoria; la pila se derrumba hasta su angulo de reposo cada vez que recibe una carga; un recuperador toma cortes de la cara expuesta y alimenta la planta. Todo lo interesante ocurre en esos dos verbos.'
        ...
        <p>
          {es
            ? 'El equipamiento es concreto. Apiladores de brazo que recorren la losa a decenas de metros por minuto. Recuperadores de puente o de rastrillo que rastrillan la seccion triangular completa de una estacion longitudinal. Ruedas de cangilones que giran y cortan un banco de un tercio del ancho. Cargadores frontales que muerden la cara accesible en una losa ROM. Camiones de 220 toneladas que vacian en unos nueve metros de diametro, no en un punto.'
        ...
        <p>
          {es
            ? 'Dos hechos duros y poco ensenados gobiernan el resultado. Primero, la geometria de apilado y la segregacion de particulas deciden la calidad de la mezcla: la cantidad de capas que un corte atraviesa fija el techo de lo que la cama puede promediar. Segundo, la trazabilidad del material recuperado es un problema real de contabilidad, y sigue sin resolverse en operacion: el articulo mas reciente sobre modelado de pilas ROM en tiempo casi real afirma explicitamente que rastrear la ley en una pila ROM con los sistemas de despacho actuales es dificil porque la informacion no esta disponible en tiempo real.'
      ...
      <section>
        <h2>{es ? '2. La fisica, en tres capas' : '2. The physics, in three layers'}</h2>
        <p>
          {es
            ? 'La GEOMETRIA es la capa mas simple y la mas consecuente. Un material granular seco no puede sostener una pendiente por encima de su angulo de reposo, asi que cada carga que llega desencadena una avalancha que reparte el exceso ladera abajo. El angulo de reposo es una propiedad del material: los valores publicados para minerales van de unos 34 grados (cobre, Noruega) a unos 60 (cobre, Peru), y se mueve con el tamano, la humedad y el tiempo desde la descarga.'
        ...
        <p>
          {es
            ? 'La SEGREGACION es la capa que hace que mezclar no sea promediar. Mientras el material avalancha, las particulas finas percolan hacia el fondo de la capa fluyente y elevan a las gruesas: el cribado cinetico. El material que se detiene primero es rico en finos y el que viaja mas lejos es grueso, de modo que el pie de la pila queda mas grueso que la cresta. Esto no es un detalle estetico: sesga lo que contiene cada corte de recuperacion segun donde y a que profundidad se tome.'
        ...
        <p>
          {es
            ? 'Cuando las dos especies tienen angulos de reposo distintos aparece una tercera capa. Makse y colaboradores mostraron que una mezcla bidispersa vertida entre dos placas se ESTRATIFICA espontaneamente en capas alternadas de gruesos y finos, siempre que los granos grandes tengan el angulo de reposo mayor; si los angulos son iguales o estan invertidos, la mezcla solo segrega. Es un resultado nitido y falsable, y es la mejor oportunidad didactica que ofrece un corte interno.'
      ...
      <section>
        <h2>{es ? '3. La matematica que gobierna' : '3. The governing mathematics'}</h2>
        <p>
          {es
            ? 'El modelo de segregacion que corre en vivo en esta aplicacion es el de Gray y Thornton. Sus velocidades de percolacion relativas al flujo global son'
        ...
        <Equation
          ...
          caption={es
            ? 'Ecuaciones (3.10) y (3.11): las particulas grandes suben a velocidad proporcional a la fraccion de finos, y las pequenas drenan a velocidad proporcional a la de gruesos. La segregacion se detiene al alcanzar una fase pura.'
        ...
        <p>
          {es
            ? 'Sustituyendo en el balance de masa de la especie fina y adimensionalizando con las escalas estandar de avalancha se obtiene la ecuacion de gobierno,'
        ...
        <Equation
          ...
          caption={es
            ? 'Ecuacion (3.18). El flujo phi(1-phi) es todo el contenido fisico: la segregacion se apaga cuando cualquiera de las especies alcanza el 100 por ciento.'
        ...
        <Equation
          ...
          caption={es
            ? 'Ecuacion (3.19), el numero de segregacion: la razon entre la velocidad media de segregacion y la magnitud tipica de la velocidad normal del flujo. En cero la ecuacion degenera a un trazador pasivo, y ese limite es el control negativo del producto.'
        ...
        <ul>
          <li><InlineMath tex="\phi" /> {es ? 'fraccion volumetrica de la especie PEQUENA, adimensional, entre 0 y 1' : 'volume fraction of the SMALL species, dimensionless, 0 to 1'}</li>
          <li><InlineMath tex="\phi_s,\ \phi_l" /> {es ? 'fracciones de la especie pequena y grande; suman uno' : 'small- and large-species fractions; they sum to one'}</li>
          ...
          <li><InlineMath tex="q" /> {es ? 'velocidad media de segregacion, m/s' : 'mean segregation velocity, m/s'}</li>
          <li><InlineMath tex="B" /> {es ? 'magnitud adimensional de la perturbacion de reparto de presion entre especies' : 'dimensionless magnitude of the pressure-partition perturbation between species'}</li>
          <li><InlineMath tex="c" /> {es ? 'coeficiente de arrastre entre particulas' : 'inter-particle drag coefficient'}</li>
          <li><InlineMath tex="\zeta" /> {es ? 'inclinacion del flanco' : 'slope inclination of the flank'}</li>
          <li><InlineMath tex="L,\ H,\ U" /> {es ? 'longitud tipica del camino, espesor de la capa fluyente y velocidad tipica' : 'typical path length, flowing-layer thickness and typical velocity'}</li>
          <li><InlineMath tex="S_r" /> {es ? 'numero de segregacion, adimensional' : 'segregation number, dimensionless'}</li>
        ...
        <p>
          {es
            ? 'La segunda ecuacion que gobierna el producto no viene de la literatura granular sino de la estadistica. Si las N capas que un corte atraviesa fueran extracciones independientes de la distribucion de entrada, la media del corte tendria varianza sigma^2_in / N, de donde'
        ...
        <Equation
          ...
          caption={es
            ? 'La cota de capas independientes, DERIVADA aqui de primeros principios y etiquetada como derivada. Una cama real recupera solo una fraccion de esta cota.'
      ...
      <section>
        ...
        <ol>
          <li>{es ? 'Un cuerpo mineralizado, sintetico y sembrado o un modelo de bloques publicado con leyes reales de cobre, da el campo de leyes.' : 'An ore body, either seeded and synthetic or a published block model with real copper grades, supplies the grade field.'}</li>
          <li>{es ? 'Una secuencia de extraccion recorre ese campo y produce un flujo de camiones: una fila por descarga, con tonelaje, leyes, fraccion gruesa, humedad y posicion.' : 'A dig sequence walks that field and produces a truck stream: one row per dump, with tonnage, grades, coarse fraction, moisture and position.'}</li>
          <li>{es ? 'El contrato de ingesta valida cada fila: rechaza lo que esta fuera de rango con una razon declarada, marca lo sospechoso y no corrige nada en silencio.' : 'The ingestion contract validates every row: it rejects what is out of range with a stated reason, flags what is suspicious, and silently coerces nothing.'}</li>
          <li>{es ? 'El metodo de apilado convierte el indice del camion en una posicion sobre la losa.' : 'The stacking method turns the truck index into a position on the pad.'}</li>
          ...
          <li>{es ? 'El libro mayor sigue la geometria: los lotes se mueven desde la cima de la columna origen a la cima de la destino, conservando su orden.' : 'The ledger follows the geometry: lots move from the top of the source column to the top of the destination, preserving their order.'}</li>
          <li>{es ? 'El recuperador toma cortes segun su geometria, y cada corte reporta la fraccion de su tonelaje proveniente de cada evento de deposicion.' : 'The reclaimer takes cuts according to its geometry, and every cut reports the fraction of its tonnage from each deposition event.'}</li>
          <li>{es ? 'Las metricas se calculan sobre base de tonelaje y se muestran contra la cota ideal y contra las anclas publicadas.' : 'The metrics are computed on a tonnage base and shown against the ideal bound and against the published anchors.'}</li>
        </ol>
      </section>
      ...
      <section>
        ...
        <Callout variant="honest" title={es ? 'Que es exacto y que no' : 'What is exact and what is not'}>
          {es
            ? 'EXACTO: la conservacion de masa (residual del orden de 1e-11 toneladas sobre un caso completo), la suma a uno de las fracciones de procedencia (1e-12), el determinismo del replay dada una semilla, la degeneracion del solucionador de segregacion a un trazador pasivo cuando Sr es cero, y la cota 1/N derivada. ILUSTRATIVO: el angulo de reposo es un valor de manual IMPUESTO, no emergente; las interfaces entre lotes son mas nitidas que en una pila real porque el remanejo y la mezcla por avalancha no estan modelados; la segregacion es un modelo continuo publicado y no verdad a escala de particula; y los datos son sinteticos salvo cuando se selecciona el carril real. Los valores publicados de VRR contra los que se compara el producto provienen de una pila CIRCULAR de otras dimensiones, asi que la prueba es ordinal y de orden de magnitud, nunca de reproduccion digito a digito.'
