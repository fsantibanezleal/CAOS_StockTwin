  ...
  return (
    <div className="page-body prose">
      <div className="page-head">
        ...
        <p className="lede">
          {es
            ? 'El diseño de validación: diecisiete casos en cinco categorías, tres de ellos controles con criterios numericos de descarte, las métricas con sus ecuaciones exactas, el protocolo de partición que impide la fuga, y las cuatro afirmaciones ordinales que el producto se compromete a publicar pasen o fallen.'
      ...
      <SubTabs orientation="vertical" ariaLabel={es ? 'Experimentos' : 'Experiments'} tabs={[
        { id: 'design', label: es ? 'Diseño' : 'Design', content: <Design es={es} /> },
...
function Design({ es }: { es: boolean }) {
  return (
    <>
      ...
      <p>
        {es
          ? 'Los casos no son ejemplos: son el diseño de validación. Cada uno declara su razón científica de inclusión, su comportamiento esperado y el criterio de descarte que dice que resultado significaria que el codigo esta mal. Sin esa última parte un caso es una demostración, no una prueba.'
      ...
      <ul>
        <li>
          <strong>{es ? 'Geometría de apilado' : 'Stacking geometry'}</strong>{' '}
          {es
            ? 'es el eje primario. El método de construcción decide cuántas capas cruza un corte, y ese es el término dominante de todo lo que sigue.'
        ...
        <li>
          <strong>{es ? 'Método de recuperación' : 'Reclaim method'}</strong>{' '}
          {es
            ? 'es independiente de la construcción: la misma pila recuperada de dos maneras da dos respuestas. Se mantiene chevron para que la comparación sea limpia, y el caso chevron con cara completa hace de referencia de este eje en vez de duplicarse como una fila propia. Una fila duplicada es relleno, no cobertura.'
        ...
        <li>
          ...
          {es
            ? 'es la autocorrelación del flujo entrante, que decide si las capas que cruza un corte son lo bastante independientes para que la cama sirva de algo. Los casos de alcance largo y con deriva son los que un producto que quisiera lucirse omitiria en silencio, así que se envian como casos destacados.'
        ...
        <li>
          <strong>{es ? 'Régimen de segregación' : 'Segregation regime'}</strong>{' '}
          {es
            ? 'cubre si el cribado cinético es lo bastante fuerte para sesgar lo que contiene cada corte, y si las dos especies tienen angulos de reposo lo bastante distintos para estratificar.'
        ...
        <li>
          ...
          {es
            ? 'son tres, cada uno con un criterio numérico. Sin ellos una respuesta plausible pero equivocada es indistinguible de una correcta.'
...
function Metrics({ es }: { es: boolean }) {
  return (
    <>
      <h2>{es ? 'Las métricas, con sus ecuaciones exactas' : 'The metrics, with their exact equations'}</h2>
      <Equation
        tex="\mathrm{VRR} = \frac{\sigma^2_{\text{out}}}{\sigma^2_{\text{in}}}, \qquad \sigma^2 = \frac{\sum_i m_i (g_i - \bar g)^2}{\sum_i m_i}"
        caption={es
          ? 'La razón de reducción sobre base de tonelaje, menor es mejor. m_i en toneladas, g_i en por ciento de cobre. Las dos varianzas deben calcularse sobre la misma base, que es el requisito explícito de Kumral.'
      ...
      <Equation
        ...
        caption={es
          ? 'La eficiencia contra la cota de capas independientes. Es la métrica que impide inflar el beneficio: una cama en 0,05 con 100 capas solo esta recuperando un quinto de lo alcanzable.'
      ...
      <Equation
        ...
        caption={es
          ? 'El índice de segregación: la diferencia de fracción gruesa entre el tercio más bajo y el más alto de la losa ocupada, por altura. Definirlo por altura y no por distancia a un centro nominal lo mantiene valido para geometrias que construyen varias crestas.'
      ...
      <Equation
        ...
        caption={es
          ? 'El residual de conservación de masa, en toneladas. Se muestra en pantalla en la App en vez de afirmarse en prosa, porque un solucionador que perdiera una fracción de un por ciento por descarga seguiria dibujando un cono convincente.'
      ...
      <p>
        {es
          ? 'Toda métrica de una corrida se reporta con una banda de credibilidad multi-semilla sobre 31 semillas, percentiles 5 y 95. Un número puntual sin intervalo es un defecto en esta línea de productos, y aquí además sería una inflación: la razón de reducción de una sola realización es a su vez una variable aleatoria, y citar la extracción que mejor se ve es la forma más fácil de exagerar lo que una cama logra.'
...
function Controls({ es }: { es: boolean }) {
  ...
  return (
    <>
      ...
      <p>
        {es
          ? 'Un control con un criterio numérico es lo que separa una prueba de una demostración. Cada uno de estos dice, por adelantado, que resultado significaria que el codigo esta mal, y el reporte de release falla si alguno no se cumple.'
      ...
      <Equation
        ...
        caption={es
          ? 'Con una losa de una sola celda no hay geometría, así que un corte es la media ponderada de toda la pila y la razón lograda debe contener la cota 1/N dentro de su banda. Si no la contiene, la implementación de la razón esta mal y todo número del producto queda en duda.'
...
function Protocol({ es }: { es: boolean }) {
  return (
    <>
      <h2>{es ? 'El protocolo de partición y la fuga que impide' : 'The split protocol and the leakage it prevents'}</h2>
      <p>
        {es
          ? 'Los sustitutos predicen un resultado de mezcla a partir de los parámetros de la corrida. Si una fila de entrenamiento y una de prueba compartieran semilla, compartirian la secuencia exacta de leyes y el modelo se evaluaria sobre datos que memorizo. Particionar solo por semilla tampoco basta: la estructura DE entrada determina la forma de todo el flujo, así que un modelo entrenado sobre todas las estructuras y evaluado sobre las mismas aprende las cuatro formas en lugar de la física. La partición es por semilla Y estructura.'
      ...
      <Equation
        ...
        caption={es
          ? 'La asignación de grupo es un hash de la clave del grupo y no una permutación, de modo que el mismo corpus se particiona igual sin importar en que orden lleguen las filas. Una prueba verifica que ningun grupo (semilla, estructura) queda a ambos lados.'
      ...
      <p>
        {es
          ? 'Los diecisiete casos de exhibición quedan retenidos por completo y ninguno aporta una fila de entrenamiento. Eso es lo que permite que la página Benchmark los reporte como evidencia en vez de como un ajuste.'
...
function Assertions({ es }: { es: boolean }) {
  return (
    <>
      ...
      <p>
        {es
          ? 'El producto se compromete con estas cuatro afirmaciones ordinales. Se escriben aquí antes de la corrida, se evaluan en la etapa de evaluación y se publican en Benchmark pasen o fallen. Un resultado negativo es un resultado: si una falla, se reporta con los numeros que la hicieron fallar, no se ajusta hasta que pase.'
      ...
      <ol>
        <li>
          <strong>A1.</strong>{' '}
          {es ? 'Chevcon mezcla mejor que los conos concéntricos, el orden en que toda la literatura coincide.' : 'Chevcon blends better than cone shell, the ordering every source agrees on.'}
        </li>
        <li>
          <strong>A2.</strong>{' '}
        ...
        <li>
          <strong>A3.</strong>{' '}
          {es ? 'Chevcon recuperado con cara completa alcanza la magnitud de la regla práctica de Bond, aproximadamente diez a uno en varianza.' : 'Chevcon reclaimed full-face reaches the magnitude of the Bond rule of thumb, about ten to one in variance.'}
        </li>
        <li>
          <strong>A4.</strong>{' '}
          {es ? 'Un rastrillo de cara completa mezcla mejor que un corte por banco, que a su vez mezcla mejor que las máquinas de alcance somero.' : 'A full-face rake blends better than a bench cut, which blends better than the shallow-reaching machines.'}
        </li>
      </ol>
      <Callout variant="honest" title={es ? 'Por qué la prueba es ordinal y no digito a digito' : 'Why the test is ordinal and not digit-for-digit'}>
        {es
          ? 'Las anclas publicadas vienen de una pila circular de otras dimensiones, y su fuente es internamente inconsistente sobre ellas: la Tabla IV de Loubser y de Korte, resuelta hacia atrás, da dos varianzas de entrada distintas para una comparación descrita como sobre la misma entrada (0,871 contra 0,620), y las conclusiones citan una razón chevcon menor a 0,1 donde la tabla dice 0,121. La conclusión ordinal esta bien respaldada; los digitos no son verdad de terreno. Reproducirlos exactamente sería una coincidencia, y afirmar que se reprodujeron sería deshonesto.'
...
function Data({ es }: { es: boolean }) {
  return (
    <>
      ...
      <p>
        {es
          ? 'El veredicto de disponibilidad se registro para que no se repita la busqueda: NO existe un registro abierto y licenciado de descargas camion por camion con leyes a la granularidad que necesita un simulador de acopio. Se buscaron Zenodo, Kaggle, el índice awesome-open-data y la literatura. El articulo más reciente sobre el tema lo dice desde dentro: rastrear la ley en una pila ROM con los sistemas de despacho actuales es difícil porque la información no está disponible en tiempo real.'
      ...
      <p>
        {es
          ? 'El replanteamiento que lo resuelve: un registro de descargas es un artefacto derivado de un modelo de bloques más una secuencia de extracción, y ambos están disponibles. El producto genera uno en vez de fingir que existe uno publico.'
        ...
        <tbody>
          <tr>
            ...
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'cuerpos mineralizados de oreblocks (Apache-2.0) más un flujo de leyes con covarianza exponencial, ambos sembrados y reproducibles bit a bit' : 'ore bodies from oreblocks (Apache-2.0) plus an exponential-covariance grade stream, both seeded and byte-reproducible'}
            </td>
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'comprometible por completo, etiquetado sintético en todas partes' : 'fully committable, labelled synthetic everywhere'}
            </td>
          </tr>
          <tr>
            <td>{es ? 'Controlado' : 'Controlled'}</td>
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'simulación gaussiana secuencial con GSTools, para barrer explicitamente el alcance y la meseta del variograma' : 'sequential Gaussian simulation with GSTools, to sweep the variogram range and sill explicitly'}
            </td>
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'carril fuera de línea; es lo que hace significativa la comparación contra la cota' : 'offline lane; it is what makes the comparison against the bound meaningful'}
            </td>
          </tr>
          <tr>
            <td>{es ? 'Real' : 'Real'}</td>
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'modelos de bloques publicados de MineLib; la instancia KD lleva una columna explícita de ley de cobre sobre 14.153 bloques' : 'published MineLib block models; the KD instance carries an explicit copper grade column over 14,153 blocks'}
            </td>
            <td style={{ whiteSpace: 'normal', textAlign: 'left' }}>
              {es ? 'descargados en tiempo de ejecución a la memoria del navegador; nunca comprometidos, nunca empaquetados, y la integración continua nunca los descarga' : 'fetched at runtime into browser memory; never committed, never bundled, and continuous integration never fetches them'}
            </td>
          </tr>
        </tbody>
      </table>
