    ...
    },
    {
      ...
      content: (
        <>
          ...
          <p className="measure">
            {t(
              'One place the mechanisms disagree, and it is not hidden. With the layer grain-limited, which it is for run-of-mine rock in every case the product runs, the segregation number scales on the path length alone, so kinetic sieving rises with drop height and FALLS gently with face angle because a steeper face is a shorter run. The sources report that steeper faces segregate more; that is ballistic trajectory segregation, which this equation does not contain. The fitted curve had merged the two and made one index rise with angle, which read as agreement with the source and was really the model having no way to disagree. The two are now reported separately: the on-face index, and the material thrown clear of the toe, which grows with angle and is almost pure coarse.',
              'Un punto donde los mecanismos discrepan, y no está escondido. Con la capa limitada por el tamaño de grano, que es el caso para roca de mina en todo lo que el producto corre, el número de segregación escala solo con la longitud del camino, así que el tamizado cinético sube con la altura de caída y BAJA suavemente con el ángulo de la cara, porque una cara más empinada es un recorrido más corto. Las fuentes reportan que las caras más empinadas segregan más; eso es segregación balística por trayectoria, que esta ecuación no contiene. La curva ajustada fusionaba ambos y hacía subir un solo índice con el ángulo, lo que se leía como acuerdo con la fuente y en realidad era el modelo sin manera de discrepar. Ahora se reportan por separado: el índice en la cara, y el material lanzado más allá del pie, que crece con el ángulo y es casi todo grueso.',
            )}
          </p>
