    ...
    },
    {
      ...
      content: (
        <>
          <p className="measure">
            {t(
              'The mechanism is kinetic sieving, identified by Savage and Lun: as granular material shears, small particles preferentially fall into the void space opening beneath them and lever the large ones upward. Gray and Thornton formulated it as a binary mixture theory, and that formulation is what runs here. It did not until engine 0.07.000, and the correction is worth stating: the solver existed, tested and complete, and nothing in any shipped path called it. What ran was three fitted curves standing in for it, while this page described the equations below. The directions those curves encoded were published and correct, which is why it went unnoticed. A release gate now walks the import graph from the bake and fails if a method rated SOTA names code the pipeline never invokes.',
              'El mecanismo es el tamizado cinético, identificado por Savage y Lun: al cortarse el material granular, las partículas pequeñas caen preferentemente en el espacio que se abre bajo ellas y empujan hacia arriba a las grandes. Gray y Thornton lo formularon como una teoría de mezcla binaria, y esa formulación es la que corre aquí. No lo hacía hasta el motor 0.07.000, y vale la pena declarar la corrección: el solucionador existía, probado y completo, y nada en el camino de producción lo llamaba. Lo que corría eran tres curvas ajustadas que lo sustituían, mientras esta página describía las ecuaciones de abajo. Las direcciones que codificaban esas curvas estaban publicadas y eran correctas, y por eso pasó inadvertido. Una compuerta de publicación ahora recorre el grafo de importaciones desde el horneado y falla si un método calificado SOTA nombra código que la tubería nunca invoca.',
            )}{' '}
            <Cite id="savage1988" paren /> <Cite id="gray2005" paren /> <Cite id="gray2006" paren />
          </p>
          <p className="measure">
            {t(
          ...
          <p className="measure">
            {t(
              'Fines drain to the BASE of the flowing layer. Material that stops on the flank is drawn from that base and material that keeps travelling is drawn from the top, so the toe, fed by what travelled furthest, ends up coarse. Coarse-at-the-toe is an OUTPUT of the model, not a rule in the code. The march deposits into stations from crest to toe and each cell of the placement takes the local coarse fraction of the station it falls in. That composition belongs to the NEW parcel only, because the ledger stores parcels: recording a size split for the material a truck has just tipped cannot restamp the older material underneath it. Species mass is conserved exactly by the deposition split, which is a test rather than a claim, and at zero segregation number every station receives the load own unsorted split.',
              'Los finos drenan a la BASE de la capa fluyente. El material que se detiene en el flanco se toma de esa base y el que sigue viajando se toma de arriba, así que el pie, alimentado por lo que viajó más lejos, termina grueso. Grueso en el pie es una SALIDA del modelo, no una regla en el código. La marcha deposita en estaciones desde la cresta hasta el pie y cada celda de la colocación toma la fracción gruesa local de la estación en que cae. Esa composición pertenece solo al lote NUEVO, porque el registro guarda lotes: anotar una granulometría para el material que un camión acaba de volcar no puede reescribir el material más viejo que está debajo. La masa de cada especie se conserva exactamente en el reparto de deposición, que es una prueba y no una afirmación, y con número de segregación cero cada estación recibe la granulometría sin clasificar de la propia carga.',
            )}
          </p>
          <p className="measure">
            {t(
              'The 2005 formulation on its own separates the species completely and then stops, and the measurement is stark: the on-face sorting index reads 0.5162 at a segregation number of 1.5 and 0.5162 at 15, identical to four decimals across a tenfold range. Every scenario would have reported the same segregation whatever its drop height. Gray and Chugunov added the missing term, the random collisional remixing that opposes the sieving, and with it the index rises across the whole operating range instead of pinning at the ceiling. The shocks survive; what remixing adds is that a shock has a finite thickness, which is what stops the answer being binary.',
              'La formulación de 2005 por sí sola separa las especies por completo y luego se detiene, y la medición es contundente: el índice de clasificación en la cara marca 0,5162 con número de segregación 1,5 y 0,5162 con 15, idéntico a cuatro decimales en un rango de diez veces. Cada escenario habría reportado la misma segregación sin importar su altura de caída. Gray y Chugunov agregaron el término que faltaba, el remezclado colisional aleatorio que se opone al tamizado, y con él el índice sube en todo el rango de operación en vez de quedar pegado al techo. Los choques sobreviven; lo que agrega el remezclado es que un choque tiene espesor finito, y eso es lo que impide que la respuesta sea binaria.',
            )}
          </p>

          <p className="measure">
            {t(
              'What that comes to on the shipped artifacts. Across 22 scenarios the per-load drop that forms a face has a median of 1.54 m and a maximum of 11.06 m, so the product runs at a segregation number between about 0.04 and 1.84, well below the ceiling, which is the reason the remixing term matters here rather than being a refinement. The on-face sorting index has a median of 0.097 and a maximum of 0.367, and on the finished piles the cell coarse fraction spans 0.256 at the 5th percentile to 0.608 at the 95th, reaching 0.98 in toe cells and 0.08 in fine-rich crest cells, with the mean at 0.365 against the 0.35 that was placed. The fitted curves this replaced produced 0.332 to 0.400: a band so narrow that the segregation half of the product had almost nothing to show.',
              'A cuánto llega esto en los artefactos publicados. En 22 escenarios la caída por carga que forma una cara tiene mediana de 1,54 m y máximo de 11,06 m, así que el producto opera con número de segregación entre 0,04 y 1,84, bien por debajo del techo, y por eso el término de remezclado importa aquí y no es un refinamiento. El índice de clasificación en la cara tiene mediana 0,097 y máximo 0,367, y en las pilas terminadas la fracción gruesa por celda va de 0,256 en el percentil 5 a 0,608 en el 95, llegando a 0,98 en celdas del pie y a 0,08 en celdas finas de la cresta, con media 0,365 frente al 0,35 colocado. Las curvas ajustadas que esto reemplaza producían de 0,332 a 0,400: una banda tan estrecha que la mitad de segregación del producto casi no tenía nada que mostrar.',
            )}
          </p>
          <p className="measure">
            {t(
              'Two regimes, and the product straddles them. The flowing layer is thickness-limited by flux below about a metre of drop and by its own grain size above it, and measured across the 16762 loads that formed a face in the shipped scenarios, 42 percent are flux-limited and 58 percent grain-limited. That matters because they do not scale alike. Flux-limited, the segregation number rises steeply with drop AND with angle, which is exactly the published direction: at half a metre of drop it runs 0.022, 0.041, 0.062, 0.071 across 35, 37, 40 and 45 degrees. Grain-limited, the thickness stops responding and only the path length is left, so a steeper face is a shorter run and the number FALLS: at eleven metres it runs 1.928, 1.838, 1.721, 1.564. The sources and the solver therefore agree where most of the material is and part company on tall faces, and none of that was put there by hand. Trajectory segregation, which is ballistic and which this equation does not contain, is reported separately as the overrun and rises with angle throughout.',
              'Dos regímenes, y el producto queda a caballo entre ambos. El espesor de la capa fluyente lo limita el flujo por debajo de un metro de caída y su propio tamaño de grano por encima, y medido sobre las 16762 cargas que formaron cara en los escenarios publicados, 42 por ciento son limitadas por flujo y 58 por ciento por grano. Importa porque no escalan igual. Limitada por flujo, el número de segregación sube fuerte con la caída Y con el ángulo, que es exactamente la dirección publicada: con medio metro de caída va 0,022, 0,041, 0,062, 0,071 entre 35, 37, 40 y 45 grados. Limitada por grano, el espesor deja de responder y solo queda la longitud del camino, así que una cara más empinada es un recorrido más corto y el número BAJA: con once metros va 1,928, 1,838, 1,721, 1,564. Las fuentes y el solucionador coinciden donde está la mayor parte del material y se separan en caras altas, y nada de eso se puso a mano. La segregación balística por trayectoria, que esta ecuación no contiene, se reporta aparte como el desborde y crece con el ángulo en todo el rango.',
            )}
          </p>
          ...
          <Callout variant="honest" title={t('What is and is not claimed', 'Lo que se afirma y lo que no')}>
            {t(
              'This is a published CONTINUUM model, not particle-scale truth. It solves no discrete elements and knows nothing of particle shape or contacts. The size distribution down the face is SOLVED, from the conservation law above; what remains anchored rather than measured is two coefficients, both named in the code and both taken from the literature rather than fitted to this material: the percolation coefficient, set so the segregation number for a reference dump lands inside the range of the source own worked examples, and the Peclet number, held at the middle of the range Gray and Chugunov fit to chute data. Where the mass lands down the face and how much overruns the toe stay published operational observations, and neither touches the size distribution. Calibrating the two coefficients with DEM or the corresponding laboratory test is recorded as future work rather than quietly assumed.',
              'Este es un modelo CONTINUO publicado, no verdad a escala de partícula. No resuelve elementos discretos ni sabe nada de forma ni contactos de partículas. La distribución de tamaños en la cara se RESUELVE, con la ley de conservación de arriba; lo que queda anclado y no medido son dos coeficientes, ambos nombrados en el código y tomados de la literatura en vez de ajustados a este material: el coeficiente de percolación, fijado para que el número de segregación de una descarga de referencia caiga dentro del rango de los ejemplos de la propia fuente, y el número de Peclet, mantenido en el medio del rango que Gray y Chugunov ajustan a datos de canaleta. Dónde cae la masa por la cara y cuánto pasa el pie siguen siendo observaciones operacionales publicadas, y ninguna toca la distribución de tamaños. Calibrar los dos coeficientes con DEM o con el ensayo de laboratorio correspondiente queda registrado como trabajo futuro en vez de suponerse hecho.',
            )}
          </Callout>
