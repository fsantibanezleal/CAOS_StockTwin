...
# Scenario `narrow_ramp`

**Narrow access ramp**

A twelve-metre corridor instead of twenty-five: single-lane access with no room to pass. The ramp still has to be cut and maintained out of the fill, with far less of it to work with.
...
> The refusal rate must be no better than the reference. A narrower way in cannot make access easier, and if it does the corridor is not the thing controlling access.
...
| access ramp | 12 m wide | reserved corridor; nothing is tipped on it, so the pile cannot grow over its own way in |
...
```bash
python data-pipeline/run.py narrow_ramp --output build/check
```
