...
**Trending feed, grade drifting through the campaign**

Grade drifts steadily from the first bench to the last, which is what a dig sequence through a zoned ore body produces. A drift is not noise: no amount of mixing removes it.
...
> Variance reduction must come out BETTER than the reference while the reclaim stream still visibly drifts. A number that improves while the problem gets worse is the point; if the number does not improve, the trend is not reaching the input.
...
```bash
python data-pipeline/run.py trending --output build/check
```
